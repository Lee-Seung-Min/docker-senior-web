    ...
    try {
      ...
      await getUserId(jwt).then(async (result) => {
        ...
        if (result != "" && result != undefined && result != "") {
          mbrId = result;
          const url =
            /*urlAddr + "8082*/ mobileUrlAddr + "/v1/favShop/selectFavDrstListMobile?favYNA=N&shpDay=" + dayType;
...
<Nav>
  ...
  <div class="notytab_wrap" slot="btns">
    ...
    <button
      type="button"
      on:click={() => {
        goto(urlList.favDrst);
      }}>단골약국</button
    >
    <button type="button" class="on">방문약국</button>
  </div>
</Nav>

<Favorite {drstList} {favType} />
