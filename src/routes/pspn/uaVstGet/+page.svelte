<script>
  ...
  function handlePhoneInput(event) {
    patient.phone = formatPhoneNumber(event.target.value);
  }
...
<section class="contents">
  ...
  <div class="box_1">
    <dl class="info_dl">
      ...
      <dd>{data.address1} {data.address2}</dd>
      <dt class="tit">전화번호</dt>
      <dd>{data.phone}</dd>
      <dt class="tit">영업시간</dt>
      <dd>{getLaucnTime(data)}</dd>
    </dl>
  </div>
...
{#if popUpWhat == "pspn"}
  <PspnPopUp {popUp}>
    ...
    <p class="btn_wrap" id="btn" slot="btns_h">
      <button type="button" class="mbtn_n_4" name="chbtn" on:click={addVst} id="close">예</button>
