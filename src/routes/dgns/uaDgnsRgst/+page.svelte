<script>
  ...
  import { chngDateHourFormat, getSecretNumFirstFormat } from "$lib/js/dateFunction";
  ...
  async function doReg() {
    day = new Date(day);
    if (hour && hour !== '' && hour !== 'vacation') {
      const [h, m] = hour.split(':');
      day.setHours(Number(h), Number(m));
    }
    dateString = chngDateHourFormat(day);
    if (who == "me") {
      dgnsPatId = 0;
    } else {
  ...
  async function searchDtr() {
    if (dept != 0) {
      dtrList = await searchDtrByDept(shpId, dept, $dgnsType);
      dtrList.forEach((dtr) => {
  ...
  $: selectedDateTime = day || hour ? `${day} ${hour}` : "";
...
<section class="contents">
  ...
  <div class="accordion-item" class:active={activeId === 'doctor'}>
    ...
    <!--캘린더는 나중에,,,, 
      1. 진료 가능 요일|진료 불가 요일 표시 -> 버튼 클릭 가능/불가능으로
  -->
  <div class="accordion-item" class:active={activeId === 'date'}>
    <button class="accordion-header" on:click={() => activeId = (activeId === 'date' ? null : 'date')}>
      <span>진료 일정 선택</span>
      {#if selectedDateTime}
        <span class="selected-text">{selectedDateTime}</span>
      {/if}
    </button>
    ...
    {#if activeId === 'date'}
      <div class="accordion-content" transition:slide>
        ...
        <div style="padding: 10px;">
          <button type="button" class="btn_01 small" style="width:100%;" on:click={() => closeAccordion('memo')}>일정 선택 완료</button>
        </div>
      </div>
    {/if}
  </div>
    ...
    <button
      ...
      on:click={() => {
        ...
        if (dept == 0) {
          ...
          text = "의사를 선택해주세요";
          popUp = true;
        } else if (hour == "" && rghour[0].htmeFrom == "holiday" && rghour[0].htmeTo == "") {
