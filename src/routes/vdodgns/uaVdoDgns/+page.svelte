<script>
  ...
  onMount(async () => {
    ...
    client
      .join("비대면진료" + dgnsId, token, memData.resultVO.dgnsPatName, pw)
      .then(async () => {
...
<section id="tel_wrap">
  ...
  <div id="tel_control">
    <div class="tools">
      <button type="button" class="hicon_video" on:click={videoCheck} id="videoButton" />
      <button type="button" class="hicon_mic" id="micButton" on:click={audioCheck} />
